...
class ComplianceService {
    static async checkUpcomingDeadlines() {
        ...
        const { data: items, error } = await database_1.supabase
    ...
    static async sendComplianceReminders() {
        ...
        for (const item of upcomingItems) {
            ...
            if ([30, 14, 7, 3, 1].includes(daysRemaining)) {
                const assignee = item.profiles;
                if (assignee?.email) {
                    await EmailService_1.EmailService.sendComplianceReminderEmail(assignee.email, item.title, item.due_date, daysRemaining);
                }
            }
        }
    }
    static async markOverdueItems() {
    ...
    static async validateComplianceRequirement(foundationId, requirementType, data) {
        const issues = [];
    ...
    static async scheduleComplianceReminders() {
        try {
