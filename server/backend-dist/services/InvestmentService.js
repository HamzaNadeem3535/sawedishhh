...
class InvestmentService {
    static async getPortfolioSummary(foundationId) {
        ...
        if (investmentsWithPerformance.length > 0) {
            summary.performance_metrics.best_performer = investmentsWithPerformance.reduce((best, current) => (current.performance || 0) > (best.performance || 0) ? current : best);
            summary.performance_metrics.worst_performer = investmentsWithPerformance.reduce((worst, current) => (current.performance || 0) < (worst.performance || 0) ? current : worst);
    ...
    static async updateInvestmentValues(foundationId) {
        const { data: investments, error } = await database_1.supabase
            .from('investments')
            .select('*')
            .eq('foundation_id', foundationId)
    ...
    static async fetchMarketData(investmentName) {
        const mockData = {
    ...
    static async calculatePortfolioRisk(foundationId) {
        const { data: investments, error } = await database_1.supabase
            .from('investments')
        ...
        riskAnalysis.diversification_score = Math.min(typeCount / 5, 1) * 100;
    ...
    static async generateInvestmentReport(foundationId, startDate, endDate) {
    ...
    static async rebalancePortfolio(foundationId, targetAllocations) {
        const portfolioSummary = await this.getPortfolioSummary(foundationId);
