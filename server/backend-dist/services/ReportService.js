...
class ReportService {
    static async generateBalanceSheet(foundationId, asOfDate) {
        const { data: accounts, error } = await database_1.supabase
            .from('accounts')
    ...
    static async generateIncomeStatement(foundationId, startDate, endDate) {
        const { data: accounts, error } = await database_1.supabase
            .from('accounts')
    ...
    static async generateCashFlowStatement(foundationId, startDate, endDate) {
    ...
    static async generateComplianceReport(foundationId) {
    ...
    static async exportReportToPDF(reportData, reportType) {
        const pdfContent = `
    ...
    static async exportReportToExcel(reportData, reportType) {
        const excelContent = `Report Type,${reportType}\nGenerated,${new Date().toISOString()}\n\n${JSON.stringify(reportData)}`;
        return Buffer.from(excelContent, 'utf-8');
    }
    static async exportReportToCSV(reportData) {
