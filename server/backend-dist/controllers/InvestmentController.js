...
class InvestmentController {
    static getInvestments = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    static getInvestmentById = (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        const { data: investment, error } = await database_1.supabase
            .from('investments')
            .select(`
        *,
        foundations(name),
      ...
      `)
            ...
            .single();
        if (error) {
            throw (0, errorHandler_1.createError)('Investment not found', 404);
        }
        res.json({ investment });
    });
    static createInvestment = (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { foundation_id, type, name, amount, currency, acquisition_date, current_value, performance, notes } = req.body;
        const { data: investment, error } = await database_1.supabase
            .from('investments')
            .insert({
            foundation_id,
    ...
    static updateInvestment = (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        const { name, amount, current_value, performance, notes } = req.body;
        const { data: investment, error } = await database_1.supabase
            .from('investments')
            .update({
            name,
    ...
    static deleteInvestment = (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
    ...
    static getInvestmentPerformance = (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
    ...
    static updateInvestmentValue = (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
